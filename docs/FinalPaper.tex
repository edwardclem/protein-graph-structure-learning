...
Pll(y, \theta) = \sum_{ij} \log \bigg[\exp( \log p(x_{ij} = 1 | x_{\Gamma(x_{ij})})) + \exp( \log p(x_{ij} = 0 | x_{\Gamma(x_{ij})})) \bigg]
\end{equation}

We replace the calculation of $F$ with $Pll$ in Algorithm $\ref{alg:mf}$. This removes the need for \textsc{mfInference} during training (although we still use \textsc{mfInference} at test time).

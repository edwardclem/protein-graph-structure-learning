function [] = run_pll(train_data, test_data, outfile, nThreads, condDist)
%runs pseudolikelihood approximation
seed = 0;
rng(seed);

% Load data
disp(train_data);
[ss_proteins, features_aa, seqlen_all, gt_train] = load_data(train_data);
L = numel(features_aa); % seqlen variable
disp(L);
N = seqlen_all.*(seqlen_all - 1)/2; % Number of possible edges

% Run Pseudolikelihood approx. CRF
...
% Setup inputs
funLL = @(theta)getLlikCRFMPll(theta, gt_train, ss_proteins, L, N, features_aa, seqlen_all, crfOpt);
...
% Plot results
%using mean-field approximation for test
